...
const createAudioVisualizer = (track, id, appendTo) => {
	try {
		...
		console.log("- Error Creating Audio Visualizer : ", error)
	}
}
	...
	try {
		...
		allStream[socket.id] = {
			video: {
				...
				id: "current-user-video",
				username: localStorage.getItem("username") ? localStorage.getItem("username") : "unknown",
				kind: "video",
				status: isCameraOn,
			},
			audio: {
				track: stream.getAudioTracks()[0],
				id: "current-user-audio",
				username: localStorage.getItem("username") ? localStorage.getItem("username") : "unknown",
				kind: "audio",
				status: isMicOn,
			},
		...
		videoParams = {
			track: stream.getVideoTracks()[0],
			...videoParams,
			appData: { picture: localStorage.getItem("picture") ? localStorage.getItem("picture") : "/assets/pictures/unknown.jpg" },
		}
		...
		console.log("- Error Get Streaming : ", error)
	}
}
...
const normalTemplate = () => {
	try {
		...
		console.log("- Error Changing To Normal Template : ", error)
	}
}
...
const screenSharingTemplate = () => {
	try {
		...
		console.log("- Error Changing To Screen Sharing Template : ", error)
	}
}
...
const videoContainerScreenSharingTrigger = (trigger) => {
	try {
		...
		console.log("- Error Triggering Chat / User List : ", error)
	}
}
...
const createDevice = async () => {
	try {
		...
		console.log("- Error Create Device Mediasoup: ", error)
...
const showLoadingScreen = () => {
	try {
		...
		console.log("- Error Showing Loading : ", error)
	}
}
...
const hideLoadingScreen = () => {
	try {
		...
		console.log("- Error Hiding Loading : ", error)
	}
}
...
const createSendTransport = () => {
	try {
			...
			producerTransport.on("connectionstatechange", async (e) => {
				...
				if (e == "connected") {
					...
					createUserList(
						...
						socket.id,
						isCameraOn,
						localStorage.getItem("picture") ? localStorage.getItem("picture") : "/assets/pictures/unknown.jpg"
					)
		...
		console.log("- Error Creating Transport Producer : ", error)
	}
}
...
const signalNewConsumerTransport = async (remoteProducerId) => {
	try {
		...
		console.log("- Error Signaling New Consumer : ", error)
	}
}
...
socket.on("change-host", ({ newHost }) => {
	try {
		...
		console.log("- Error Changing Host : ", host)
	}
})
...
function checkIfUserIsDisplayed(videoProducerId, audioProducerId, isMicActive, maxAttempts = 10, socketId) {
	try {
		...
		console.log("- Error Checking If User Is Displayed : ", error)
	}
}
...
socket.on("mic-config", (data) => {
	try {
		...
		console.log("- Error In Mic Config Socket : ", error)
	}
})
...
socket.on("screen-sharing", ({ videoProducerId, audioProducerId, isSharing, remoteProducerId }) => {
	try {
		...
		console.log("- Error Screen Sharing Socket : ", error)
	}
})
...
socket.on("new-producer", ({ producerId }) => {
	try {
		...
		console.log("- Error Informing New Producer : ", error)
	}
})
				...
				if (counter >= startDisplay && counter <= endDisplay) {
					for (const secondKey in allStream[firstKey]) {
						...
						if (allStream[firstKey][secondKey].kind != "screen-sharing" && allStream[firstKey][secondKey].kind == "video" && !checkVideo) {
							...
							createVideo(
								allStream[firstKey][secondKey].id,
								secondKey,
								allStream[firstKey][secondKey].track,
								allStream[firstKey][secondKey].username,
								allStream[firstKey].audio.track.enabled,
								authority,
								allStream[firstKey].video.status,
								allStream[firstKey].video.picture
							)
	...
	try {
		...
		if (kind == "video") {
			...
			if (remoteId == "current-user-video") {
				...
				newElem.innerHTML =
					'<div class="icons-mic"><img src="/assets/pictures/' +
					isMic +
					'" class="mic-image" id="local-mic"/></div><video id="' +
					remoteId +
					'" autoplay class="user-video" muted></video>' +
					isVideo +
					'<div class="username" id="my-username">' +
					newUsername +
					"</div>"
				...
				newElem.innerHTML =
					'<div class="icons-mic"><img src="/assets/pictures/' +
					isMic +
					'" poster="/assets/pictures/unknown.jpg" preload="auto" class="mic-image" /></div><video id="' +
					remoteId +
					'" autoplay class="user-video" ></video>' +
					isVideo +
					'<div class="username">' +
					newUsername +
					"</div>"
...
const createImageTrack = async (imagePath) => {
	try {
		...
		console.log("- Error Creating Image Track : ", error)
	}
}
...
const createVideoTrackFromImageTrack = async (imageTrack) => {
	try {
		...
		console.log("- Error Creating Video From Image Track : ", error)
	}
}
...
const createScreenSharing = (track) => {
	try {
		...
		console.log("- Error Creating Screen Sharing : ", error)
	}
}
...
const createPaginationRight = () => {
	try {
		...
		console.log("- Error Creating Right Pagination : ", error)
	}
}
...
const createPaginationLeft = () => {
	try {
		...
		console.log("- Error Creating Left Pagination : ", error)
	}
}
...
const isScreenSharingType = (input) => {
	try {
		...
		console.log("- Error Checking Screen Sharing Type : ", error)
	}
}
				...
				consumerTransport.on("connectionstatechange", async (e) => {
					...
					if (e == "connected") {
						...
						if (
							params.kind == "video" &&
							!check &&
							((totalUsers <= limitedPerPage && currentPage == 0) ||
								(totalUsers >= limitedPerPage && totalPage2 == currentPage + 1 && currentPage != 0))
						) {
						...
						if (!allStream[params.producerOwnerSocket][params.kind] && !check) {
							allStream[params.producerOwnerSocket][params.kind] = {
								track,
								id: remoteProducerId,
						...
						if (check) {
							...
							if (!screenSharingVideo) {
								...
								if (!allStream[params.producerOwnerSocket].screenSharing) {
									allStream[params.producerOwnerSocket].screenSharing = {
										track,
										id: remoteProducerId,
										username: params?.username,
										kind: "screen-sharing",
						...
						if (!producersDetails[params.producerOwnerSocket]) {
							...
							if (
								producersDetails[params.producerOwnerSocket] &&
								!check &&
								(!producersDetails[params.producerOwnerSocket].video || !producersDetails[params.producerOwnerSocket].audio)
							) {
								// User Joins +1
							}
							...
							if (!producersDetails[params.producerOwnerSocket][params.kind] && !check) {
								...
								if (!allStream[socket.id].video.status) {
									socket.emit("camera-config", {
										socketId: params.producerOwnerSocket,
										isCameraActive: false,
										videoProducerId: videoProducer.id,
										videoStreamId: socket.id,
									})
								}
						...
						if (
							params.kind == "video" &&
							!check &&
							((currentPage == 0 && totalUsers <= limitedPerPage) ||
								(totalUsers >= limitedPerPage && totalPage == currentPage + 1 && currentPage != 0))
						) {
							socket.emit("consumer-resume", { serverConsumerId: params.serverConsumerId })
						}
					}
...
micButton.addEventListener("click", () => {
	try {
		...
		console.log("- Error At Mic Button : ", error)
	}
})
...
switchCamera.addEventListener("click", () => {
	try {
		...
		console.log("- Error At Switch Camera Button : ", error)
	}
})
const SwitchingCamera = async () => {
	try {
		...
		console.log("- Error At Switching Camera : ", error)
	}
}
...
const isVideoDisplayed = async (data) => {
	try {
		...
		console.log("- Error At Checking User List Camera Icon : ", error)
	}
}
...
const turnOffCamera = async () => {
	try {
		...
		console.log("- Error Turning Off Cameera : ", error)
	}
}
...
const turnOnCamera = async () => {
	try {
		...
		console.log("- Error Turning On Cameera : ", error)
	}
}
...
socket.on("camera-config", (data) => {
	try {
		...
		console.log("- Error At Camera Config Socket : ", error)
	}
})
...
turnOnOffCamera.addEventListener("click", async () => {
	try {
		...
		console.log("- Error At Turn On / Off Camera Button : ", error)
	}
})
...
screenSharingButton.addEventListener("click", () => {
	try {
		...
		console.log("- Error At Screen Sharing Button : ", error)
	}
})
...
recordButton.addEventListener("click", () => {
	try {
		...
		console.log("- Error At Record Button : ", error)
	}
})
...
chatButton.addEventListener("click", () => {
	try {
		...
		console.log("- Error At Chat Button : ", error)
	}
})
...
userListButton.addEventListener("click", () => {
	try {
		...
		console.log("- Error At User List Button : ", error)
	}
})
...
const muteAllParticipants = () => {
	try {
		...
		console.log("- Error Muting All Participants Socket Emit : ", error)
	}
}
...
const unlockAllMic = () => {
	try {
		...
		console.log("- Error Unlocking All Participants Socket Emit : ", error)
	}
}
